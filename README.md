...
a small, browser-renderable preview **inline** (no forced download). It tries,
in order: a cached thumbnail → the RAW embedded preview → an **OS "system"
thumbnail** → the original bytes.

- **RAW** (`.arw/.cr2/.cr3/.nef/.dng/…`) → the JPEG preview **embedded** in the
  file, extracted with a zero-dep marker scan and cached at
  `<sidecar>/<id>.thumb.jpg`. (A browser can't decode raw sensor data, so
  without this RAW files would show as blank placeholders.)
- **Everything else** (JPEG/PNG/WebP/GIF/AVIF, EXR, RAW with no embedded
  preview) → the same small thumbnail your file manager shows, generated by the
  **OS thumbnailer** and cached at `<sidecar>/<id>.systhumb.png`:
  - **macOS** → QuickLook (`qlmanage -t`)
  - **Windows** → the Shell thumbnail (`IShellItemImageFactory`, the previews
    Explorer shows — RAW/HEIC included when a codec is installed)
  - **Linux** → `gdk-pixbuf-thumbnailer` (GNOME) when present

  This keeps the library grid light — e.g. a 20 MB JPEG becomes a ~400 KB
  preview. The first request per file spawns the OS tool (cached afterward, so
  the initial load of a large folder is a one-time cost). If the OS tool is
  missing or can't handle a type, the server falls back to the original bytes
  (for raster) or a 404 (EXR/etc.). Pass `--no-system-thumbnails` to disable and
  serve originals instead.
...
- **Thumbnail cache** → `<sidecar>/<id>.thumb.jpg` (RAW embedded preview) or
  `<sidecar>/<id>.systhumb.png` (OS thumbnail); safe to delete, regenerated on
  demand.
